-- Acaedu: Add missing columns + Secure RLS policies
-- This migration adds created_by columns and applies secure RLS
...
END $$;
DO $$ BEGIN
  ALTER TABLE profiles ADD COLUMN year_of_study INT;
EXCEPTION WHEN duplicate_column THEN NULL;
END $$;

-- Add venue fields if missing
DO $$ BEGIN
  ALTER TABLE venues ADD COLUMN floor TEXT;
EXCEPTION WHEN duplicate_column THEN NULL;
END $$;
DO $$ BEGIN
...
DO $$ BEGIN
  ALTER TABLE schedules ADD COLUMN is_cancelled BOOLEAN DEFAULT false;
EXCEPTION WHEN duplicate_column THEN NULL;
END $$;
DO $$ BEGIN
  ALTER TABLE schedules ADD COLUMN cancel_reason TEXT;
EXCEPTION WHEN duplicate_column THEN NULL;
END $$;
DO $$ BEGIN
...
END $$;
DO $$ BEGIN
  ALTER TABLE schedules ADD COLUMN alarm_minutes_before INT DEFAULT 15;
EXCEPTION WHEN duplicate_column THEN NULL;
END $$;

-- Add notification field if missing
DO $$ BEGIN
  ALTER TABLE notifications ADD COLUMN color_tag TEXT DEFAULT 'blue';
EXCEPTION WHEN duplicate_column THEN NULL;
END $$;

-- Add event fields if missing
DO $$ BEGIN
...
-- Add announcement field if missing
DO $$ BEGIN
  ALTER TABLE announcements ADD COLUMN target_audience TEXT DEFAULT 'all';
EXCEPTION WHEN duplicate_column THEN NULL;
END $$;

-- Add semester to videos if missing
DO $$ BEGIN
  ALTER TABLE videos ADD COLUMN semester TEXT;
EXCEPTION WHEN duplicate_column THEN NULL;
END $$;

-- ═══════════════════════════════════════════════════════════
-- CREATE TABLES IF NOT EXIST (for new deployments)
-- ═══════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════
-- SECURE RLS POLICIES (authenticated-only, owner-restricted)
-- ═══════════════════════════════════════════════════════════
...
-- Apply triggers (ignore if already exist)
DO $$ BEGIN
