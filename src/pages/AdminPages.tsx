    ...
    <div ref={ref} className="flex items-end gap-2 h-36 px-1">
      {data.map((d, i) => (
        <div key={i} className="flex-1 flex flex-col items-center gap-1">
          <span className="text-[10px] font-bold text-navy">{d.value}</span>
          ...
          <span className="text-[9px] text-text-muted text-center leading-tight">{d.label}</span>
        </div>
      ))}
    </div>
  )
    ...
    <div className="flex flex-col items-center gap-2">
      <div className="rounded-full relative" style={{ width: size, height: size, background: `conic-gradient(${gradients.join(', ')})` }}>
        <div className="absolute rounded-full bg-white flex items-center justify-center"
          ...
          <span className="text-sm font-extrabold text-navy" style={{ fontFamily: 'Plus Jakarta Sans, sans-serif' }}>{total}</span>
        </div>
      </div>
    ...
    <div>
      ...
      {loading ? (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {Array.from({length: 8}).map((_,i) => <div key={i} className="h-28 rounded-xl bg-beige/50 animate-pulse"/>)}
        </div>
      ) : (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {cards.map((card, i) => (
            <motion.div key={i} initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:i*0.05}}
              ...
              <div className="text-2xl font-extrabold text-navy" style={{ fontFamily: 'Plus Jakarta Sans, sans-serif' }}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        ...
        <motion.div initial={{opacity:0,x:-20}} animate={{opacity:1,x:0}} transition={{delay:0.3}}
          className="p-6 rounded-xl bg-white border border-beige">
          <div className="text-xs font-semibold text-gold uppercase tracking-[0.15em] mb-1">Enrollment Trends</div>
          <p className="text-xs text-text-muted mb-4">Breakdown by status</p>
          {loading ? <div className="h-36 bg-beige/50 rounded-lg animate-pulse"/> : (
            <AdminBarChart
              data={[
        ...
        <motion.div initial={{opacity:0,x:20}} animate={{opacity:1,x:0}} transition={{delay:0.4}}
          className="p-6 rounded-xl bg-white border border-beige flex flex-col">
          <div className="text-xs font-semibold text-gold uppercase tracking-[0.15em] mb-1">Revenue Overview</div>
          <p className="text-xs text-text-muted mb-4">Payment status distribution</p>
          {loading ? <div className="h-36 bg-beige/50 rounded-lg animate-pulse"/> : (
          ...
          <div className="mt-4 pt-4 border-t border-beige flex items-center justify-between">
            ...
            <span className="text-sm font-extrabold text-navy" style={{ fontFamily: 'Plus Jakarta Sans, sans-serif' }}>
      ...
      <motion.div initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{delay:0.5}}
        className="p-6 rounded-xl bg-white border border-beige">
        <div className="text-xs font-semibold text-gold uppercase tracking-[0.15em] mb-1">User Distribution</div>
        <p className="text-xs text-text-muted mb-4">Breakdown by role</p>
        {loading ? <div className="h-36 bg-beige/50 rounded-lg animate-pulse"/> : (
          <div className="flex flex-col md:flex-row items-center gap-8">
            ...
            <div className="flex-1 grid grid-cols-3 gap-4 w-full">
              {[
                ...
                <div key={i} className="p-4 rounded-lg bg-cream text-center">
