    ...
    <div>
      ...
      {showForm && (
        <motion.div initial={{opacity:0,y:-8,scale:0.98}} animate={{opacity:1,y:0,scale:1}} transition={{duration:0.25}}
          ...
          <h3 className="text-[15px] font-bold mb-5" style={{ fontFamily: 'var(--font-display)', color: 'var(--color-navy)' }}>Upload Lecture Video</h3>
      ...
      {loading ? (
        ...
        <motion.div initial={{opacity:0,scale:0.95}} animate={{opacity:1,scale:1}} className="empty-state">
          ...
          <p style={{ color: 'var(--color-text-muted)' }}>No lecture videos yet.</p>
        </motion.div>
      ) : (
          ...
          {videos.map((v) => (
            <motion.div key={v.id} variants={fadeUp}
              ...
              {playingId === v.id ? (
                ...
                <div className="aspect-video flex items-center justify-center cursor-pointer relative"
                  style={{ background: 'var(--color-bg-secondary)' }}
                  onClick={() => setPlayingId(v.id)}>
                  <motion.div whileHover={{scale:1.1}} whileTap={{scale:0.95}}
                    className="w-14 h-14 rounded-full flex items-center justify-center"
                    style={{ background: 'color-mix(in srgb, var(--color-primary) 12%, transparent)' }}>
                    <Play size={24} className="ml-0.5" style={{ color: 'var(--color-primary)' }}/>
                  </motion.div>
              ...
              <div className="p-4">
                <h3 className="font-bold mb-1" style={{ fontFamily: 'var(--font-display)', color: 'var(--color-navy)' }}>{v.title}</h3>
                <p className="text-xs" style={{ color: 'var(--color-text-muted)' }}>{v.description}</p>
                <div className="flex items-center gap-3 mt-2">
                  <span className="flex items-center gap-1 text-xs" style={{ color: 'var(--color-text-muted)' }}><Clock size={12}/> {v.semester || 'N/A'}</span>
  ...
  return (
    <div>
      ...
      {showForm && (
        <motion.div initial={{opacity:0,y:-8,scale:0.98}} animate={{opacity:1,y:0,scale:1}} transition={{duration:0.25}}
          className="card p-6 mb-6">
          <h3 className="text-[15px] font-bold mb-5" style={{ fontFamily: 'var(--font-display)', color: 'var(--color-navy)' }}>Upload Material</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5">
      ...
      {loading ? (
        ...
        <motion.div initial={{opacity:0,scale:0.95}} animate={{opacity:1,scale:1}} className="empty-state">
          ...
          <p style={{ color: 'var(--color-text-muted)' }}>No materials uploaded yet.</p>
        </motion.div>
      ) : (
        <motion.div variants={stagger} initial="hidden" animate="show" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {materials.map((m) => (
            <motion.div key={m.id} variants={fadeUp}
              ...
              <div className="flex items-center gap-3 mb-2">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="font-bold truncate" style={{ fontFamily: 'var(--font-display)', color: 'var(--color-navy)' }}>{m.title}</h3>
                  <span className="badge badge-navy">{m.file_type}</span>
                </div>
              </div>
              <p className="text-xs mb-3 line-clamp-2" style={{ color: 'var(--color-text-muted)' }}>{m.description}</p>
              {m.file_url && <a href={m.file_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-xs font-semibold hover:underline" style={{ color: 'var(--color-primary)' }}><Download size={12}/> Download</a>}
            </motion.div>
          ))}
        </motion.div>
      )}
