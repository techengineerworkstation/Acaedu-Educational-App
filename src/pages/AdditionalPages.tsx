...
export function ProfilePage({ user }: { user: User }) {
  ...
  useEffect(() => {
    fetchTable('profiles', { id: user.id }).then(p => setProfile(p[0] || null)).finally(() => setLoading(false))
  }, [user.id])
  ...
  return (
    <div>
      ...
      <div className="max-w-2xl">
        <div className="p-6 rounded-xl bg-white border border-beige mb-6">
          <div className="flex items-center gap-4 mb-6">
            <div className="w-20 h-20 rounded-full bg-navy/10 flex items-center justify-center text-navy font-bold text-3xl" style={{ fontFamily: 'Plus Jakarta Sans, sans-serif' }}>{user.full_name?.[0] || '?'}</div>
            <div>
              <h2 className="text-xl font-extrabold text-navy" style={{ fontFamily: 'Plus Jakarta Sans, sans-serif' }}>{user.full_name}</h2>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="flex items-center gap-3 p-3 rounded-lg bg-cream">
              ...
              <div><div className="text-xs text-text-muted">Email</div><div className="font-medium text-sm">{user.email}</div></div>
            </div>
            <div className="flex items-center gap-3 p-3 rounded-lg bg-cream">
              <Phone size={18} className="text-navy"/>
              <div><div className="text-xs text-text-muted">Phone</div><div className="font-medium text-sm">{(profile?.phone as string) || 'Not set'}</div></div>
            </div>
            <div className="flex items-center gap-3 p-3 rounded-lg bg-cream">
              <BookOpen size={18} className="text-navy"/>
              <div><div className="text-xs text-text-muted">Department</div><div className="font-medium text-sm">{(profile?.department as string) || 'Not set'}</div></div>
            </div>
            <div className="flex items-center gap-3 p-3 rounded-lg bg-cream">
              <UserIcon size={18} className="text-navy"/>
              <div><div className="text-xs text-text-muted">Role</div><div className="font-medium text-sm capitalize">{user.role}</div></div>
            </div>
          </div>
        </div>
      </div>
