    ...
    <div>
      ...
      {showForm && (
        <motion.div initial={{opacity:0,y:-8,scale:0.98}} animate={{opacity:1,y:0,scale:1}} transition={{duration:0.25}}
          ...
          <h3 className="text-[15px] font-bold mb-5" style={{ fontFamily: 'var(--font-display)', color: 'var(--color-navy)' }}>Create Announcement</h3>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-start gap-2">
                      <h3 className="font-bold" style={{ fontFamily: 'var(--font-display)', color: 'var(--color-navy)' }}>{a.title as string}</h3>
                      <span className={`badge ${config.badge} flex-shrink-0`}>{a.priority as string}</span>
                    </div>
                    <p className="text-sm mt-1" style={{ color: 'var(--color-text-muted)' }}>{a.content as string}</p>
                    <div className="text-xs mt-2" style={{ color: 'var(--color-text-muted)' }}>{a.created_at ? new Date(a.created_at as string).toLocaleDateString() : ''}</div>
                  </div>
                </div>
              </motion.div>
            )
